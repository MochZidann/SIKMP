...
import com.kopdes.kopdesjajar.ui.owner.OwnerDashboardFragment
import com.kopdes.kopdesjajar.ui.kasir.KasirReportsFragment
import com.kopdes.kopdesjajar.ui.kasir.KasirProductsFragment
import com.kopdes.kopdesjajar.ui.kasir.KasirPosFragment
import com.kopdes.kopdesjajar.ui.kasir.KasirDashboardFragment
import com.kopdes.kopdesjajar.ui.admin.UserManagementFragment
import com.kopdes.kopdesjajar.ui.admin.PromoConfigFragment
import com.kopdes.kopdesjajar.ui.admin.MemberManagementFragment
import com.kopdes.kopdesjajar.ui.admin.KoperasiProfileFragment
import com.kopdes.kopdesjajar.ui.admin.DatabaseManagementFragment
import com.kopdes.kopdesjajar.ui.admin.AuditLogFragment
import com.kopdes.kopdesjajar.ui.admin.AdminDashboardFragment
...
class DashboardActivity : BaseAuthedActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun defaultDestination(role: Role?): Pair<Int, Fragment> {
        return when (role) {
            ...
            Role.ADMIN_GUDANG -> R.id.nav_gudang_dashboard to com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangDashboardFragment()
    ...
    private fun onNavigationItemSelected(item: MenuItem, args: Bundle?): Boolean {
        ...
        val fragment: Fragment? = when (item.itemId) {
            ...
            R.id.nav_gudang_dashboard -> com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangDashboardFragment()
            R.id.nav_gudang_products -> com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangProductsFragment()
            R.id.nav_gudang_categories -> com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangCategoriesFragment()
            R.id.nav_gudang_stock -> com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangStockFragment()
            R.id.nav_gudang_reports -> com.kopdes.kopdesjajar.ui.admin_gudang.AdminGudangReportsFragment()
            ...
            R.id.nav_owner_sales_report -> com.kopdes.kopdesjajar.ui.owner.OwnerSalesReportFragment()
            R.id.nav_owner_stock_report -> com.kopdes.kopdesjajar.ui.owner.OwnerStockReportFragment()
            R.id.nav_owner_inventory_health -> com.kopdes.kopdesjajar.ui.owner.OwnerInventoryHealthFragment()
    ...
    private fun startPeriodicSync() {
        periodicSyncJob = lifecycleScope.launch(Dispatchers.IO) {
            ...
            while (isActive) {
                try {
                    Log.d("SyncDebug", "🔄 [AutoSync] Memulai Push & Pull Otomatis...")
                    syncManager.pushAllDataToServer()
                    syncManager.pullAllDataFromCloud()
                    Log.d("SyncDebug", "✅ [AutoSync] Push & Pull Selesai.")
                    withContext(Dispatchers.Main) {
                        notifyActiveFragmentReload()
                    }
                } catch (e: Exception) {
                    Log.e("SyncDebug", "❌ [AutoSync] Gagal sinkronisasi otomatis: ${e.message}")
                }
                delay(30000) // Sinkronisasi setiap 30 detik
            }
        }
    }

    private fun notifyActiveFragmentReload() {
        val fragment = currentFragment ?: return
        Log.d("SyncDebug", "🔄 Mengirim sinyal reload data ke fragment: ${fragment.javaClass.simpleName}")
        try {
