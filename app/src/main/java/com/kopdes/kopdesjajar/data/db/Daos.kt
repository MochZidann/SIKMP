...
interface MemberDao {
    ...
    fun getUnsynced(): List<MemberEntity>
...
interface StockMovementDao {
    ...
    fun getUnsynced(): List<StockMovementEntity>
...
interface AuditLogDao {
    ...
    fun getUnsynced(): List<AuditLogEntity>
...
interface PromoDao {
    ...
    fun getUnsynced(): List<PromoEntity>
...
interface SalesDao {
    ...
    fun getUnsynced(): List<SaleEntity>
...
internal class MemberDaoImpl(private val helper: KoperasiDbHelper) : MemberDao {
    ...
    override fun getUnsynced(): List<MemberEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM members WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toMember() }
        }
    }
    ...
    override fun updateSyncStatus(id: Long, isSynced: Boolean) {
        val db = helper.writableDatabase
        val cv = ContentValues().apply { put("isSynced", if (isSynced) 1 else 0) }
        db.update("members", cv, "id = ?", arrayOf(id.toString()))
    }
...
internal class ProductDaoImpl(private val helper: KoperasiDbHelper) : ProductDao {
    ...
    override fun getUnsynced(): List<ProductEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM products WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toProduct() }
        }
    }
    ...
    override fun updateStockAbsolute(productId: Long, stock: Long) {
        val db = helper.writableDatabase
        db.execSQL(
            "UPDATE products SET stock = ?, isSynced = 0 WHERE id = ?",
...
internal class StockMovementDaoImpl(private val helper: KoperasiDbHelper) : StockMovementDao {
    ...
    override fun getUnsynced(): List<StockMovementEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM stock_movements WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toStockMovement() }
        }
    }
    ...
    override fun updateSyncStatus(id: Long, isSynced: Boolean) {
        val db = helper.writableDatabase
        val cv = ContentValues().apply { put("isSynced", if (isSynced) 1 else 0) }
        db.update("stock_movements", cv, "id = ?", arrayOf(id.toString()))
    }
...
internal class SettingsDaoImpl(private val helper: KoperasiDbHelper) : SettingsDao {
    ...
    override fun upsert(settings: SettingsEntity) {
        ...
        val cv = ContentValues().apply {
            ...
            put("koperasiPhone", settings.koperasiPhone)
            ...
            put("isSynced", if (settings.isSynced) 1 else 0)
    ...
    override fun updateSyncStatus(isSynced: Boolean) {
        val db = helper.writableDatabase
        val cv = ContentValues().apply { put("isSynced", if (isSynced) 1 else 0) }
...
internal class AuditLogDaoImpl(private val helper: KoperasiDbHelper) : AuditLogDao {
    ...
    override fun getUnsynced(): List<AuditLogEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM audit_logs WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toAudit() }
        }
    }
    ...
    override fun updateSyncStatus(id: Long, isSynced: Boolean) {
        val db = helper.writableDatabase
        val cv = ContentValues().apply { put("isSynced", if (isSynced) 1 else 0) }
        db.update("audit_logs", cv, "id = ?", arrayOf(id.toString()))
    }
...
internal class PromoDaoImpl(private val helper: KoperasiDbHelper) : PromoDao {
    ...
    override fun getUnsynced(): List<PromoEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM promos WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toPromo() }
        }
    }
    ...
    override fun updateSyncStatus(id: Long, isSynced: Boolean) {
        val db = helper.writableDatabase
        val cv = ContentValues().apply { put("isSynced", if (isSynced) 1 else 0) }
        db.update("promos", cv, "id = ?", arrayOf(id.toString()))
    }
...
internal class SalesDaoImpl(private val helper: KoperasiDbHelper) : SalesDao {
    ...
    override fun getUnsynced(): List<SaleEntity> {
        val db = helper.readableDatabase
        db.rawQuery("SELECT * FROM sales WHERE isSynced = 0", null).use { c ->
            return c.toList { it.toSale() }
        }
    }
    ...
    override fun bestSeller(fromEpochMs: Long, toEpochMs: Long, category: String?): BestSeller? {
        ...
        return if (category.isNullOrBlank()) {
            ...
            db.rawQuery(
                "SELECT si.productName as productName, COALESCE(SUM(si.quantity), 0) as qty FROM sale_items si INNER JOIN sales s ON s.id = si.saleId WHERE s.createdAtEpochMs BETWEEN ? AND ? AND p.category = ? AND s.status = 'SUCCESS' GROUP BY si.productName ORDER BY qty DESC LIMIT 1",
...
private fun Cursor.toSettings(): SettingsEntity {
    return SettingsEntity(
        ...
        koperasiPhone = getString(getColumnIndexOrThrow("koperasiPhone")),
